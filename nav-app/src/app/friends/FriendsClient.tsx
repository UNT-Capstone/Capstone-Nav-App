...
export default function FriendsClientPage() {
  ...
  const { data: currentUserData = [] } = useQuery(trpc.getUsers.queryOptions(undefined));
  ...
  const { data: friendIds = [] } = useQuery(trpc.getFriendIds.queryOptions(undefined));

  const filteredSearchResults = searchResults.filter(
    (user) => user.id !== currentUserId && !friendIds.includes(user.id) && !sentRequestIds.includes(user.id)
  );
        ...
        return (
          <div className="space-y-4">
            {filteredSearchResults.length > 0 ? (
              filteredSearchResults.map((user) => (
                <Card key={user.id} className="p-4 hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-4">
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleSendFriendRequest(user.id)}>
                      Add Friend
                    </Button>
                  </div>
                </Card>
              ...
              <div className="text-center text-gray-500 py-8">No users found matching "{searchQuery}"</div>
            ) : null}
          </div>
        );
        ...
        return (
          <div className="space-y-4 max-h-96 overflow-y-auto">
            {isLoadingFriends ? (
              ...
              friends
                .filter(
                  (friend) => !searchQuery.trim() || friend.name.toLowerCase().includes(searchQuery.toLowerCase())
                )
                .map((friend) => (
              ...
              <div className="text-center text-gray-500 py-8">No friends yet. Start by finding some friends!</div>
            )}
          </div>
        );
          ...
          <div className="space-y-4">
            {isLoadingRequests ? (
              ...
              friendRequests
                .filter(
                  (request) =>
                    !searchQuery.trim() || request.sender.name.toLowerCase().includes(searchQuery.toLowerCase())
                )
                .map((request) => (
              ...
              <div className="text-center text-gray-500 py-8">No pending friend requests.</div>
            )}
          </div>
        );
